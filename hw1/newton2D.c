...
double  newton(double *x, double *y)
{
  ...
  while(err>=EPSILON){
   ...
   ynew = yold + k;
   err = root(fabs((xnew - xold)*(xnew - xold)+(ynew - yold)*(ynew - yold))); // Compute the difference.  (fabs(xnew - xold)+fabs(ynew - yold)); root(fabs((ynew*ynew+xnew*xnew)+(yold*yold+xold*xold)-x*(ynew*yold+xnew*xold))); root(fabs((xnew - xold)*(xnew - xold)+(ynew - yold)*(ynew - yold)))
   xold = xnew; // Save current value.
   yold = ynew; // Save current value.  修改過
   i ++;
  }   
